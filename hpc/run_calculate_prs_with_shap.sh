...
#SBATCH -o  /nfs/scratch/projects/ukbiobank/err_out/%A_calculate_prs_with_shap.out
#SBATCH -e /nfs/scratch/projects/ukbiobank/err_out/%A_calculate_prs_with_shap.err
...
#if [ ! -f "$PHENO_PATH/scores/featureShapValues.all.csv" ]; then
#   echo "LD not completed for featureShapValues.all.csv"
#   #run LD script
...
#if [ ! -f "$PHENO_PATH/scores/featureShapValues.all.csv" ]; then
#   echo "LD analysis failed for featureShapValues.all.csv"
FEATURES_FOR_MODEL_FILE="$PHENO_PATH/scores/featureShapValues.all.csv"
#   echo "reverting to original featureShapValues.all.csv"
#else
...
if [ -f "$FEATURES_FOR_MODEL_FILE" ]; then
    ...
    if head -1 "$FEATURES_FOR_MODEL_FILE" | grep -q "shap_zscore"; then
        echo "shap_zscore column found. Filtering features with shap_zscore > $threshold or shap_zscore < -$threshold..."
        ...
        if [ $? -eq 0 ] && [ -f "$FILTERED_FEATURES_FILE" ]; then
            ...
            echo "Filtered features (|z_score| > $threshold) or (|z_score| < -$threshold): $FILTERED_COUNT"
...
export FEATURE_SCORES_FILE=$FEATURES_FOR_MODEL_FILE
python "$SCRIPTS_DIR/prs_with_shapley.py" 
